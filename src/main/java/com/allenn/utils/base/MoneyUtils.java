...
public class MoneyUtils {
	 ...
	 */
	  ...
	  * 汉语中货币单位大写
	  */
	 private static final String[] CN_UPPER_MONETARY_UNIT = { "分", "角", "元","拾", "佰", "仟", "万", "拾",
		 													  "佰", "仟", "亿", "拾", "佰", "仟", "兆", "拾",
		 													  "佰", "仟" };
	 /**
	  * 特殊字符：整
	 ...
	 /**
	  * @Description: 人民币转换为大写,格式为：x万x千x百x十x元x角x分
	  * @autor: allen wang
	  * @Date: 2016-06-22
	  *
	  ...
	  * @return
	  */
	 public static String number2CNMonetary(String numberOfMoney) {
		 return number2CNMonetary(new BigDecimal(numberOfMoney));
	 }
	 ...
	 * @return
	 */
	public static String number2CNMonetary(BigDecimal numberOfMoney) {
	...
	/**
	 * @Description: 将人民币转换为会计格式金额(xxxx,xxxx,xxxx.xx),保留两位小数
	 * @autor: allen wang
	 * @Date: 2016-06-22
	 *
	 * @param money		待转换的金额
	...
	/**
	 * @Description: 格式化金额，显示为xxx万元，xxx百万,xxx亿
	 * @autor: allen wang
	 * @Date: 2016-06-22
	 *
	 * @param money 	待处理的金额
	...
	/**
	 * @Description: 获取会计格式的人民币(格式为:xxxx,xxxx,xxxx.xx)
	 * @autor: allen wang
	 * @Date: 2016-06-22
	 *
	 * @param money 	待处理的金额
	...
	/**
	 * @Description: 将人民币转换为会计格式金额(xxxx,xxxx,xxxx.xx)
	 * @autor: allen wang
	 * @Date: 2016-06-22
	 *
	 * @param money 	待处理的金额
	...
	private static String accountantMoney(BigDecimal money, int scale, double divisor){
        ...
        String exceptDotMoney = null;//小数点之前的字符串
        String dotMoney = null;//小数点之后的字符串
        if(dotPosition > 0){  
        ...
        StringBuffer returnMoney = new StringBuffer();//返回值
        for(int i = 0; i < moneyChar.length; i++){  
            if(i != 0 && i % 3 == 0){
				returnMoney.append(",");//每隔3位加','
            }
			returnMoney.append(moneyChar[i]);
        }
		returnMoney.reverse();//字符串倒转
        if(dotPosition > 0){
